...
void setup()
{
  ...
  byte error, address, TOF_Address;
  int nDevices;
  uint8_t byteData = 0;
  uint16_t wordData = 0;
 ...
 // uses a 7-bit address without the R/W bit instead (0x29 or 0b0101001).  
  ...
  if(TOF_discovered_flag == 1)
  {
    ...
    else
    {
      ...
      Serial.printf("TOF i2C address (DEC): %d \n", TOF_Address);
      ...
      if (!status_int)
      {
        status_int = VL53L1_SetDistanceMode(Dev, VL53L1_DISTANCEMODE_LONG);
        //sensor.setDistanceMode(DistanceMode mode)
      }
      else
      {
        //Serial.printf("SetDistanceMode failure: %d \n", status_int);
      }

      if(!status_int)
      {
        status_int = VL53L1_SetMeasurementTimingBudgetMicroSeconds(Dev, (uint32_t)MEASUREMENT_BUDGET_MS * 1000);
        //sensor.setMeasurementTimingBudget(uint32_t budget_us)
      }
      else
      {
        Serial.printf("SetDistanceMode failure: %d \n", status_int);
      ...
      if(!status_int)
      {
        status_int = VL53L1_SetInterMeasurementPeriodMilliSeconds(Dev, INTER_MEASUREMENT_PERIOD_MS);
        //sensor.startContinuous(uint32_t period_ms) Starts continuous ranging measurements. The specified inter-measurement period in milliseconds determines how often the sensor takes a measurement; if it is shorter than the timing budget, the sensor will start a new measurement as soon as the previous one finishes.
      }
      else
      {
        Serial.printf("SetMeasurementTimingBudgetMicroSeconds failure: %d \n", status_int);
        
      }

      if(!status_int)
      {
        status_int = VL53L1_StartMeasurement(Dev);
      }
      else
      {
          
        Serial.printf("SetInterMeasurementPeriodMilliSeconds failure: %d \n", status_int);  
      }
      
      //if (!status_int) status_int = VL53L1_SetMeasurementTimingBudgetMicroSeconds(Dev, (uint32_t)MEASUREMENT_BUDGET_MS * 1000);
      //if (!status_int) status_int = VL53L1_SetInterMeasurementPeriodMilliSeconds(Dev, INTER_MEASUREMENT_PERIOD_MS);
      //if (!status_int) status_int = VL53L1_StartMeasurement(Dev);
      //sensor.startContinuous(50);

/*
 from https://github.com/pololu/vl53l1x-arduino/blob/master/README.md
 setROISize(uint8_t width, uint8_t height)
 getROISize(uint8_t * width, uint8_t * height)
 setROICenter(uint8_t spadNum)
 getROICenter()
 ...
 */
      ...
      if (status_int)
      {
        Serial.printf("StartMeasurement failure: %d \n", status_int); 
      }
      else
      {
