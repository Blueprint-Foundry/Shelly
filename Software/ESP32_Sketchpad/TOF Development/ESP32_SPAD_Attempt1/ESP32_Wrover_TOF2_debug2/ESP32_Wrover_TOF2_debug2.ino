...
- adapted "ESP32 get chip ID example from: https://github.com/espressif/arduino-esp32/tree/master/libraries/ESP32/examples/ChipID/GetChipID

April 25, 2021 3:13am
- introduce "#include <VL53L1X.h>" and "VL53L1X sensor;" and the lines at the start:
  1. "device_i2c_address = sensor.getAddress(); 
  2. Serial.printf("Device address:  %d \n",device_i2c_address);" 
  lines to see if i2c really works
- btw, notice how there is no "VL53L1X.h" file in https://github.com/pololu/vl53l1x-st-api-arduino/blob/master/vl53l1x-st-api/vl53l1x-st-api.ino, 
  but there is in https://www.youtube.com/watch?v=b18ghRCWawo 
...
void setup() {
  ...
  Wire.begin(4, 13); //Join I2C bus, set SDA and SCL pins respectively
  ...
  Serial.printf("VL53L1X I2C address is 0x41:  %d \n",device_i2c_address);
...
void loop() 
{
  ...
  for (i = 0; i < 16; i++)
  {
    ...
    if (!status_int)
    {
      ...
      if (status_int)
      {
        //Serial.print(F("VL53L1_GetRangingMeasurementData failed: \n"));
        Serial.printf("VL53L1_GetRangingMeasurementData failed, status: %d \n", status_int);
        delay(5000);
      }
